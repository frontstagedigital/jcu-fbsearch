...
// small SSJS-safe utils we rely on from your environment
var Safe = (typeof Safe !== "undefined") ? Safe : {
...
var Html = (typeof Html !== "undefined") ? Html : {
  ...
  esca: function(s){ return Html.esc(s).replace(/"/g, "&quot;"); },
  buffer: function(){
    var out = [];
    return { add: function(s){ out.push(String(s)); }, toString: function(){ return out.join(""); } };
  }
};
...
// ========== CountBar (SSJS-safe, self-contained) ==========
var CountBar = (function () {
  ...
  function render(api, G) {
    ...
    var queryTerm = Safe.get(api, "response.resultPacket.query", "");
    var hasQueryTerm = (queryTerm != null && String(queryTerm).trim() !== "");
    var b = Html.buffer();
    ...
    if (hasQueryTerm) {
      b.add(' for "<span id="search-query-term">' + Html.esc(queryTerm) + '</span>"');
    }
    b.add('</div>');
...
var Results = (function () {
  ...
  function description150(result) {
    var md = (result && result.listMetadata) || {};
    var salesPitch = (intOnlyFlag === true) 
      ? firstNonEmptyArray(md.salesPitchInternational, md.salesPitchDomestic)
      : firstNonEmptyArray(md.salesPitchDomestic);
